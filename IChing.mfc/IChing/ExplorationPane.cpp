...
BEGIN_MESSAGE_MAP(CExplorationPane, CWnd)
	ON_WM_CREATE()
	ON_WM_SIZE()
END_MESSAGE_MAP()

CExplorationPane::CExplorationPane()
{
}
...
int CExplorationPane::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if(CWnd::OnCreate(lpCreateStruct) != -1)
	{
		m_pfrmExploration = CreateFrameDocView(RUNTIME_CLASS(CFrameWnd),RUNTIME_CLASS(CExplorationCtrl),NULL,true);
		m_pfrmExploration->ModifyStyleEx(0, WS_EX_STATICEDGE);
		return 0;
	}
	return -1;
}

CFrameWnd* CExplorationPane::CreateFrameDocView(CRuntimeClass* pFrameClass, CRuntimeClass* pViewClass, CDocument* pDocument, bool bNoBorder)
{
	CView* pView = NULL;
...
void CExplorationPane::OnSize(UINT nType, int cx, int cy)
{
	CWnd::OnSize(nType, cx, cy);
	if(m_pfrmExploration->GetSafeHwnd())
		m_pfrmExploration->MoveWindow(0, 0, cx, cy);
}
